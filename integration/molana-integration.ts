...
import { DataSlice, GetProgramAccountsFilter, MemcmpFilter, SYSVAR_CLOCK_PUBKEY } from '@solana/web3.js'
...
export const MOLANA_USER_TRADE_VAULT_TAG = Buffer.from('molana-user-trade-vault')
...
export const SOLANA_CONNECTION = new anchor.web3.Connection(config.rpcEndpoint, {
...
export const initMolanaProgram = (
  ...
  const result: IResult = { success: true, data: null, msg: '' }
  try {
    programId = pid
    const provider = new anchor.Provider(connection, wallet, anchor.Provider.defaultOptions())

    // Generate the program client from IDL.
    program = new (anchor as any).Program(idl, programId, provider) as anchor.Program<Molana>
  } catch (e: any) {
...
export const setTraderAccount = async (
  ...
  try {
    const molanaTrade = await pda([MOLANA_TRADE_TAG], programId)
    const molanaTreasury = await pda([MOLANA_TREASURY_TAG], programId)
    const traderAccount = await pda([MOLANA_TRADER_ACCOUNT_TAG, signer.toBuffer()], programId)
...
export const setDepositRouter = async (
  ...
  if (amount.toNumber() < MIN_DEPOSIT_AMOUNT || amount.toNumber() >= MAX_DEPOSIT_AMOUNT) {
    result.success = false
    result.msg = 'invalid amount. check min & max amount!'
    return result
  }
  try {
    const paymentIdSeed = paymentId.toArrayLike(Buffer, 'be', 8)
    const depositRouter = await pda([MOLANA_DEPOSIT_ROUTER_TAG, paymentIdSeed, signer.toBuffer()], programId)
    const tradeMint = await pda([MOLANA_TRADE_MINT_TAG, Buffer.from(currency)], programId)
    const tradeTreasury = await pda([MOLANA_TRADE_TREASURY_TAG, tradeMint.toBuffer()], programId)
    ...
    console.log('tx = ', tx)
    result.msg = 'successfully set transaction id = ' + paymentId + ' of deposit-router'
  } catch (e: any) {
    result.success = false
    result.msg = e.message
  }
...
export const setDepositState = async (
  ...
  signer: anchor.web3.PublicKey = program.provider.wallet.publicKey,
  // signer: anchor.web3.PublicKey = new anchor.web3.PublicKey('BJMXfGdK2MsrmfTQZKAjonL64AKnduKV48ximxSozxwu'),
  signers: anchor.web3.Keypair[] = []
) => {
  let result: IResult = { success: true, data: null, msg: '' }
  if (amount.toNumber() < MIN_DEPOSIT_AMOUNT || amount.toNumber() >= MAX_DEPOSIT_AMOUNT) {
  ...
  try {
    ...
    for (key in state) {
      result.msg = "successfully set deposit-router's state as " + key
      break
    }
  } catch (e: any) {
    result.success = false
...
export const supplyDepositTx = async (
  ...
  try {
    const molanaTrade = await pda([MOLANA_TRADE_TAG], programId)
    const molanaTreasury = await pda([MOLANA_TREASURY_TAG], programId)
    const depositRouterData = await program.account.depositRouter.fetch(depositRouter)
    ...
    console.log('tx = ', tx)
    result.msg = 'successfully supplied ' + depositRouterData.amount.toNumber() + ' ' + currency
  } catch (e: any) {
    result.success = false
    result.msg = e.message
  }
...
export const setWithdrawRouter = async (
  ...
  if (amount.toNumber() > MAX_WITHDRAW_AMOUNT || amount.toNumber() < MIN_WITHDRAW_AMOUNT) {
    result.success = false
    result.msg = 'invalid amount. check min & max amount!'
    return result
  }
  try {
    ...
    const withdrawIdSeed = withdrawId.toArrayLike(Buffer, 'be', 8)
    const molanaTreasury = await pda([MOLANA_TREASURY_TAG], programId)
    const tradeMint = await pda([MOLANA_TRADE_MINT_TAG, Buffer.from(currency)], programId)
    const tradeTreasury = await pda([MOLANA_TRADE_TREASURY_TAG, tradeMint.toBuffer()], programId)
    const tradeVault = await pda([MOLANA_TRADE_VAULT_TAG, tradeMint.toBuffer()], programId)
    ...
    const tx = await program.rpc.setWithdrawRouter(amount, destination, currency, {
      accounts: {
        molanaTrade,
        tradeTreasury,
        withdrawRouter,
...
export const createUserTreasury = async (
  ...
  try {
    const traderAccount = await pda([MOLANA_TRADER_ACCOUNT_TAG, signer.toBuffer()], programId)
    const tradeTreasury = await pda([MOLANA_TRADE_TREASURY_TAG, tradeMint.toBuffer()], programId)
    const userTreasury = await pda([MOLANA_USER_TREASURY_TAG, tradeTreasury.toBuffer(), signer.toBuffer()], programId)
...
export const setSolOffer = async (
  ...
  try {
    ...
    const userTreasuryA = await pda([MOLANA_USER_TREASURY_TAG, tradeTreasuryA.toBuffer(), signer.toBuffer()], programId)
    const userTreasuryB = await pda([MOLANA_USER_TREASURY_TAG, tradeTreasuryB.toBuffer(), signer.toBuffer()], programId)
    const solOffer = await pda([MOLANA_OFFER_TAG, userTreasuryA.toBuffer(), userTreasuryB.toBuffer()], programId)
...
export const sellSolOffer = async (
  ...
  try {
    ...
    const userTreasuryAData = await program.account.userTreasury.fetch(userTreasuryA)
    const userTreasuryBData = await program.account.userTreasury.fetch(userTreasuryB)
    const tradeTreasuryA = userTreasuryAData.tradeTreasury
    const tradeTreasuryB = userTreasuryBData.tradeTreasury
    const tradeTreasuryAData = await program.account.tradeTreasury.fetch(tradeTreasuryA)
    const tradeTreasuryBData = await program.account.tradeTreasury.fetch(tradeTreasuryB)
    ...
    console.log('tx = ', tx)
    const showAmount = await getShowingTokenAmount(amount, tradeTreasuryBData.tradeMint)
...
export const buySolOffer = async (
  ...
  try {
    ...
    console.log('tx = ', tx)
    const showAmount = await getShowingTokenAmount(amount, tradeTreasuryAData.tradeMint)
    result.msg = 'successfully traded for ' + showAmount
  } catch (e: any) {
    result.success = false
    result.msg = e.message
...
export const setSolEthOffer = async (
  ...
  try {
    ...
    const solEthOffer = await pda([MOLANA_SOL_ETH_OFFER_TAG, solEthOfferSeed.toBuffer(), signer.toBuffer()], programId)
    const molanaTrade = await pda([MOLANA_TRADE_TAG], programId)
...
export const acceptSolEthOffer = async (
  ...
  try {
    ...
    const tradeTreasuryData = await program.account.tradeTreasury.fetch(tradeTreasury)
    const tradeVault = tradeTreasuryData.tradeVault
    const traderAccount = await pda([MOLANA_TRADER_ACCOUNT_TAG, userTreasuryData.owner.toBuffer()], programId)
    const solEthTrading = await pda([MOLANA_SOL_ETH_TRADING_TAG, signer.toBuffer(), solEthOffer.toBuffer()], programId)
    const tx = await program.rpc.acceptSolEthOffer(acceptState, amount, lockPeriod, aVault, {
      accounts: {
        molanaTreasury,
        solEthOffer,
        solEthTrading,
    ...
    console.log('tx = ', tx)
    result.msg = 'you accepted offer - ' + solEthOffer.toBase58() + ' successfully'
  } catch (e: any) {
    result.success = false
    result.msg = e.message
  }
...
export const setSolEthTradingState = async (
  ...
  try {
    ...
    console.log('tx = ', tx)
    result.msg = 'successfully updaed for offer - ' + solEthOffer.toBase58() + ' with ' + state.toString()
  } catch (e: any) {
    result.success = false
    result.msg = e.message
  }
...
export const setSolEthTx = async (
  ...
  try {
    ...
    console.log('tx = ', tx)
    result.msg = 'successfully set transaction hash for offer - ' + solEthOffer.toBase58()
  } catch (e: any) {
    result.success = false
    result.msg = e.message
  }
...
export const setEthBscOffer = async (
  ...
  try {
    ...
    const ethBscOffer = await pda([MOLANA_ETH_BSC_OFFER_TAG, ethBscOfferSeed.toBuffer(), signer.toBuffer()], programId)
    const molanaTrade = await pda([MOLANA_TRADE_TAG], programId)
...
export const acceptEthBscOffer = async (
  ...
  try {
    ...
    const ethBscTrading = await pda([MOLANA_ETH_BSC_TRADING_TAG, signer.toBuffer(), ethBscOffer.toBuffer()], programId)
    const molanaTrade = await pda([MOLANA_TRADE_TAG], programId)
    const molanaTreasury = await pda([MOLANA_TREASURY_TAG], programId)
...
export const setEthBscTradingState = async (
  ...
  try {
    const molanaTrade = await pda([MOLANA_TRADE_TAG], programId)
    const ehtBscTradingData = await program.account.ethBscTrading.fetch(ethBscTrading)
...
export const setEthBscTraderTx = async (
  ...
  try {
    ...
    const ethBscOffer = ehtBscTradingData.offer
    const userTreasury = ehtBscTradingData.userTreasury
    const tradeTreasury = await pda([MOLANA_USER_TREASURY_TAG, userTreasury.toBuffer(), signer.toBuffer()], programId)
...
// ------------------------------untility functions------------------------
export async function getShowingTokenAmount(amount: anchor.BN, mintKey: anchor.web3.PublicKey) {
  const token = new Token(program.provider.connection, mintKey, TOKEN_PROGRAM_ID, program.provider.wallet as any)
...
export const getTraderAccount = async (owner: anchor.web3.PublicKey = program.provider.wallet.publicKey) => {
  const traderAccount = await pda([MOLANA_TRADER_ACCOUNT_TAG, owner.toBuffer()], programId)
  return await program.account.traderAccount.fetchNullable(traderAccount)
}
export const getMultipleTraderAccounts = async (owners: [anchor.web3.PublicKey]) => {
  const traderAccounts = []
  for (let i = 0; i < owners.length; i++) {
    const traderAccount = await pda([MOLANA_TRADER_ACCOUNT_TAG, owners[i].toBuffer()], programId)
...
export const getMultipleSolEthOffers = async (offers: [anchor.web3.PublicKey]) => {
...
export const getAllSolEthOffers = async (owner?: anchor.web3.PublicKey, splToken?: anchor.web3.PublicKey) => {
  const accountName = 'solEthOffer'
  const discriminator = anchor.AccountsCoder.accountDiscriminator(accountName)
  const filters: any[] = []
  if (owner) {
...
export const getMultipleEthBscOffers = async (offers: [anchor.web3.PublicKey]) => {
...
export const getMultipleSolEthTradings = async (tradings: [anchor.web3.PublicKey]) => {
  return await program.account.solEthTrading.fetchMultiple(tradings)
}
export const getSolEthTradingsByOwner = async (owner: anchor.web3.PublicKey) => {
...
export const getMultipleEthBscTradings = async (tradings: [anchor.web3.PublicKey]) => {
  return await program.account.ethBscTrading.fetchMultiple(tradings)
}
export const getEthBscTradingsByOwner = async (owner: anchor.web3.PublicKey) => {
...
export const getDepositRouterByTx = async (
  ...
  const paymentIdSeed = paymentId.toArrayLike(Buffer, 'be', 8)
  const depositRouter = await pda([MOLANA_DEPOSIT_ROUTER_TAG, paymentIdSeed, owner.toBuffer()], programId)
  return await program.account.depositRouter.fetchNullable(depositRouter)
}
export const getMultipleDepositRouters = async (routers: [anchor.web3.PublicKey]) => {
  return await program.account.depositRouter.fetchMultiple(routers)
}
export const getDepositRoutersByOwner = async (owner: anchor.web3.PublicKey) => {
...
export const getWithdrawRouterByWithdrawId = async (
  ...
  const paymentIdSeed = withdrawId.toArrayLike(Buffer, 'be', 8)
  const withdrawRouter = await pda([MOLANA_WITHDRAW_ROUTER_TAG, paymentIdSeed, owner.toBuffer()], programId)
  return withdrawRouter
}
export const getMultipleWithdrawRouters = async (routers: [anchor.web3.PublicKey]) => {
  return await program.account.withdrawRouter.fetchMultiple(routers)
}
export const getWithdrawRoutersByOwner = async (owner: anchor.web3.PublicKey, offset?: number, length?: number) => {
...
export const getMultipleTradeVaults = async (vaults: [anchor.web3.PublicKey]) => {
...
export const getMultipleUserTreasuries = async (routers: [anchor.web3.PublicKey]) => {
  return await program.account.userTreasury.fetchMultiple(routers)
}
export const getUserTreasuryByOwner = async (owner: anchor.web3.PublicKey) => {
...
export const getVaultAmount = async (address: anchor.web3.PublicKey) => {
  const tokenAccount = await getTokenAccount(program.provider.connection, address)
...
export const getDepositRouterIdByTx = async (
  txid: number,
  owner: anchor.web3.PublicKey = program.provider.wallet.publicKey
) => {
  const paymentId = new anchor.BN(txid)
...
export const getWithdrawRouterByTx = async (wId: number) => {
  const withdrawId = new anchor.BN(wId)
  const withdrawIdSeed = withdrawId.toArrayLike(Buffer, 'be', 8)
  const withdrawRouter = await pda([MOLANA_WITHDRAW_ROUTER_TAG, withdrawIdSeed], programId)
